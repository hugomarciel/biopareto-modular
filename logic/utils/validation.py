...
# (Contiene la lógica de lectura y procesamiento que estaba en data_management.py y app.py)
...
def validate_and_process_fronts(contents_list, filename_list, current_data):
    """
    Validates and processes uploaded JSON files into front data structures.
    This function was moved from data_management.py to centralize file processing logic.
    Returns (updated_data, status_children, main_objectives).
    """
    ...
    # Asegurar que contents y filenames sean listas
    ...
    for contents, filename in zip(contents_list, filename_list):
        ...
        # 1. Validación de estructura con la utilidad simple
        is_valid, msg = validate_json_structure(data)
        if not is_valid:
            errors.append(f"{filename}: {msg}")
            continue

        # 2. Extracción y procesamiento (se repite la lógica de app.py)
        objectives = []
        explicit_objectives = []
        ...
        if 'num_genes' not in objectives:
            objectives.append('num_genes') # Se añade siempre al final si existe el campo

        # 3. Validación de consistencia de objetivos
        if updated_data["main_objectives"] is None or not updated_data["fronts"]: 
            ...
            # Validar contra los objetivos principales
